package io.github.nicksen.assets;
...
public class AssetServlet extends HttpServlet {
    ...
    private static final MediaType DEFAULT_MEDIA_TYPE = HTML_UTF_8;
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            try {
                cachedAsset = loadAsset(builder.toString());
            } catch (IllegalArgumentException e) {
                cachedAsset = loadAsset("/");
            }
            ...
            if (null != mimeTypeOfExtension) {
                try {
                    mediaType = MediaType.parse(mimeTypeOfExtension);
                    if (null != defaultCharset && mediaType.is(ANY_TEXT_TYPE)) {
    ...
    private CachedAsset loadAsset(String key) throws URISyntaxException, IOException {
        ...
        final String absoluteRequestedResourcePath = SLASHES.trimFrom(resourcePath + requestedResourcePath);
