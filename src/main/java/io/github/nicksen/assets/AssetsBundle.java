...
public class AssetsBundle implements Bundle {
    ...
    @Override
    public void initialize(Bootstrap<?> bootstrap) {/* unused */}

    @Override
    public void run(Environment environment) {
        log.info("Registering AssetBundle with name: {} for path {}", assetsName, uriPath + '*');
        environment.servlets().addServlet(assetsName, createServlet()).addMapping(uriPath + '*');
    }

    private AssetServlet createServlet() {
        return new AssetServlet(resourcePath, uriPath, indexFile, UTF_8);
    }
}
